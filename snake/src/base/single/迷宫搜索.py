...
class Maze(object):
    ...
    def __init__(self, mazeFile):
        ...
        self.t.shape(
            'turtle')  # 给当前指示点设置样式(类似鼠标箭头)，海龟形状为参数指定的形状名，指定的形状名应存在于TurtleScreen的shape字典中。多边形的形状初始时有以下几种："arrow", "turtle", "circle", "square", "triangle", "classic"。
        self.wn = turtle.Screen()  # 创建一个能在里面作图的窗口
        self.wn.setworldcoordinates(-columnsInMaze / 2, -rowsInMaze / 2, columnsInMaze / 2,
                                    rowsInMaze / 2)  # 设置世界坐标系，原点在迷宫正中心。参数依次为画布左下角x轴坐标、左下角y轴坐标、右上角x轴坐标、右上角y轴坐标
    ...
    def moveTurtle(self, x, y):
        ...
        self.t.setheading(self.t.towards(x + self.xTranslate,
                                         -y + self.yTranslate))  # setheading()设置海龟朝向，towards()从海龟位置到由(x, y)，矢量或另一海龟位置连线的夹角。此数值依赖于海龟初始朝向，由"standard"、"world"或"logo" 模式设置所决定。
    ...
    def isExit(self, row, col):
        return (
                row == 0 or row == self.rowsInMaze - 1 or col == 0 or col == self.columnsInMaze - 1)  # 根据海龟位置是否在迷宫的4个边线位置判断
...
def search_from(maze, startRow, startColumn):
    ...
    # 4. 依次尝试每个方向
    found = search_from(maze, startRow - 1, startColumn) or \
            search_from(maze, startRow + 1, startColumn) or \
            search_from(maze, startRow, startColumn - 1) or \
            search_from(maze, startRow, startColumn + 1)
    if found:  # 找到出口
...
if __name__ == '__main__':
    ...
    c_maze = Maze(data)
    c_maze.drawMaze()
    search_from(c_maze, c_maze.startRow, c_maze.startCol)
